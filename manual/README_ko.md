...
### 2.2 `TCBL` 대비 향상된 특징
- 외부 CLI 도구 뿐 아니라 단일 파일을 처리하는 Python 프로그램을 플러그인으로 추가하여 사용할 수 있습니다. (`bmp2png`, `mozjpeg`, `group_md5`, `remove_bom`, `mp3lame` 의 5개 플러그인을 기본 제공합니다.)
...
- 사전에 정의된 파라미터 값 중에서 사용자가 간편하게 커서키로 세팅을 고를 수 있는 기능을 제공합니다.
...
- 파일관리자인 토탈 커맨더(Total Commander)에서는 파라미터로` %UL` 을 전달하면, 토탈커맨더에서 선택한 파일들의 목록이 임시 폴더에 리스트 파일로 만들어져서 전달됩니다.
```listfile
# 주석은 # 으로 시작
...
한 예로, 대부분의 MP3 인코더는 전체 스트림(스테레오 음악이라면 2채널 분량)의 전송율을 지칭하지만, HMP3 인코더는 채널당 전송율로 지정합니다. 즉, lameenc 에서 128bps는 HMP3에서 64kbps에 해당합니다. 상기 toml 설정 예제에서
`label="128kbps", value=64`로 기재된 것은 이 때문입니다.
...
| `mp3lame` | FileSession | WAV/FLAC/OGG/AIFF → MP3 인코딩 (LAME, `mp3lame` 플러그인) | `bitrate` (int, preset) |
...
- **v2.7:** 번들 플러그인 `mp3lame` 추가 — WAV/FLAC/OGG/AIFF를 LAME으로 MP3 인코딩 (`soundfile`로 디코딩 후 `lameenc`로 인코딩)
- **v2.8:** 번들 플러그인 `group_md5`의 그룹핑 로직 변경.
 - 정규식으로 노이즈(가변 구간/미보호 순번)만 제거한 정규화 제목의 완전일치로 그룹 판별.
 - 화이트 리스트를 도입하여 보호식별자 추가 (AAA001, AAA002는 같은 AAA그룹으로 인식하지만 IPZZ100, IPZZ101은 IPZZ가 화이트 리스트에 들어있어서 다른 그룹으로 인식함.)
 - 화이트 리스트 미발견의 경우 AAA001 패턴 외 다른 문자열이 발견되면(예: AAA001 Movie Title Name.mp4)개별 그룹, AAA001 단독 패턴으로 발견되면 AAA를 그룹화.
 - 품번패턴 마지막의 언더바(_)들은 노이즈로 판단하고 제거.
 - 전체 파일 카운터 외에 그룹내 파일 카운터 추가.
- **v2.9:** 번들 플러그인 `group_md5` 버전업 (v1.5->v1.6). 품번 바로 뒤에 붙는 노이즈 인정 범위를 언더바 전용에서 "영문자 1개(a-z/A-Z) + 언더바(들)"까지 확대하여, `...START612v`, `...START612v_`, `...START612v__` 같은 파일명도 `...START612`와 같은 그룹으로 묶이도록 개선.
- **v3.0:** 코드 리뷰 결과를 반영한 보안/견고성 강화
 - 플러그인 이름 화이트리스트 검증 추가 (`^[\w-]+$`) — `plugin = "..."` 값에 경로 조작 문자가 들어가면 `load_plugin()` 진입 전에 즉시 거부 (`core/plugins.py`)
 - 콘솔/로그 출력 직전 제어 문자(ESC 등)를 제거 — 파일명·메시지가 신뢰할 수 없는 소스(네트워크 공유, 압축 해제 결과 등)에서 와도 터미널 이스케이프 인젝션·로그 위조로 이어지지 않도록 방어 (`core/display.py`)
 - ACP 우회 경로용 임시 디렉터리 생성을 수동 `mkdir()`에서 `tempfile.mkdtemp()`로 변경 — 원자적 생성 + 소유권 강제로 symlink/TOCTOU 공격에 더 안전 (`core/executor.py`)
 - output이 input과 같은 경로로 resolve되는 설정 실수를, `validate_config.py` 사전 검증을 건너뛰었더라도 실행 중에 경고 로그로 남기도록 개선 (`allow_output_overwrite`로 opt-out 가능)
 - `core/logging.py`를 `core/logger.py`로 이름 변경 — 표준 라이브러리 `logging` 모듈과의 이름 충돌 우려 해소
