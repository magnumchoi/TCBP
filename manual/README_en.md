...
[TOC]
## 1. Overview
This tool is a Python-based reimplementation of functionality inspired by [TCBL (Total Commander Batch Builder & Launcher)](https://totalcmd.net/plugring/TCBL_1.02.html).
...
- In addition to external CLI tools, you can add a Python program that processes a single file as a plugin. (Five plugins — `bmp2png`, `mozjpeg`, `group_md5`, `remove_bom`, `mp3lame` — are bundled by default.)
...
- The program relies on the Python standard library only. The following optional packages are used automatically when available:
...
The default output language is Korean. This setting applies only to text generated by the program itself (errors, warnings, logs, and similar output). User-defined content in `config.toml`, such as `desc` or `{ msg = "..." }`, is preserved exactly as written and is not translated. The language is determined according to the following priority:
...
- A parameter already supplied via CLI `key=value` skips its interactive prompt.
- However, if a parameter with a `preset` receives a CLI value, its range is still validated against the preset. A value outside the allowed range exits with an error.
- A parameter with a `default` value but without a `preset` continues to accept free-form input; however, the prompt is prefilled with the default value so pressing `Enter` alone confirms it. If neither `preset` nor `default` is specified, the previous behavior remains unchanged.
...
**Mitigating confusion caused by a mismatch between the selected label and the stored value**
A `preset`'s `value` may differ from the value recognized by the user. For example, `ch_bitrate` is specified as a per-channel rate, so selecting "128kbps" may actually store the value `64`. In such cases, the following practices are recommended.
1. **Include the real value in the label text** — for example, `label="128kbps (64kbps/ch)"`, so the label itself clarifies the actual meaning of the stored value.
2. **Use the `{key}_label` placeholder** — for every parameter declared with `preset`, `{key}_label` (the text of the label the user selected) is generated automatically and may be used inside `pre`/`post`/`commands` entries that use `{ msg = "..." }`.
   ```toml
   { msg = "   Bitrate : {ch_bitrate_label} -> {ch_bitrate} kbps/channel" },
   ```
   The final confirmation screen also automatically pairs the selected label with the stored value for any parameter chosen via `preset`: `ch_bitrate = 64  (selected: 128kbps (64kbps/ch))`.
...
| `mp3lame` | FileSession | Encodes WAV/FLAC/OGG/AIFF to MP3 (LAME, the `mp3lame` plugin) | `bitrate` (int, preset) |
...
python tcbp.py mp3lame   list.txt bitrate=192
```
...
python plugin\mp3lame.py    <input> <output> [bitrate=128]
```
...
| `mp3lame` | `lameenc`, `soundfile`, `numpy` |
...
- **v2.7:** Added the bundled `mp3lame` plugin — encodes WAV/FLAC/OGG/AIFF to MP3 with LAME (decoded via `soundfile`, encoded via `lameenc`).
- **v2.8:** Changed the grouping logic of the bundled `group_md5` plugin. 
 - Groups are now determined by exact match of a normalized title, produced by stripping only known noise (variable segments / unprotected sequence numbers) via regex. 
 - Added a whitelist for hyphen-less AV part numbers so they're recognized as protected identifiers (AAA001 and AAA002 are recognized as the same AAA group, but IPZZ100 and IPZZ101 are recognized as separate groups because IPZZ is registered in the whitelist).
 - For a label not on the whitelist, if other text remains in the file name after removing the match, it's treated as an individual (protected) group; if the file name is nothing but the bare AAA001-style pattern, it's grouped by the letters as AAA.
 - Underscore(s) trailing right after a part number are now treated as noise and stripped.
 - Added a within-group file counter alongside the overall file counter.
- **v2.9:** Version-bumped the bundled `group_md5` plugin (v1.5 -> v1.6). Widened the trailing-noise rule glued right after a part number from underscore-only to "a single letter (any of a-z/A-Z) + underscore(s)", so filenames like `...START612v`, `...START612v_`, and `...START612v__` are now grouped together with `...START612`.
- **v3.0:** Security/robustness hardening from a code review
 - Added a whitelist check for plugin names (`^[\w-]+$`) — a `plugin = "..."` value containing path-manipulation characters is now rejected immediately, before `load_plugin()` proceeds (`core/plugins.py`)
 - Strip control characters (ESC, etc.) right before console/log output — protects against terminal escape injection / log forging even when a filename or message comes from an untrusted source (a network share, extracted archive contents, etc.) (`core/display.py`)
 - Switched the ACP-bypass temp directory from a manual `mkdir()` to `tempfile.mkdtemp()` — atomic creation with enforced ownership is safer against symlink/TOCTOU attacks (`core/executor.py`)
 - A config mistake where output resolves to the same path as input now logs a runtime warning even if `validate_config.py`'s pre-run check was skipped (opt out via `allow_output_overwrite`)
 - Renamed `core/logging.py` to `core/logger.py` to remove the name collision with the standard library's `logging` module
